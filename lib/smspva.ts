...
export async function getAllPricesForService(
  ...
  try {
    ...
    // Documented as { scode, sname, clist: [] } but the item shape isn't
    // spelled out, so accept the plausible spellings and bail out (rather
    // than returning an empty map) if none match - the caller falls back to
    // the slow per-country path instead of showing "no countries".
    const list = data?.data?.clist ?? data?.data?.countries ?? data?.data;
    if (!Array.isArray(list) || list.length === 0) return null;

    const out = new Map<string, number>();
    for (const item of list) {
      const code = item?.country ?? item?.Country ?? item?.ccode ?? item?.code;
      const price = Number(item?.price ?? item?.Price ?? item?.cost);
      if (typeof code === "string" && Number.isFinite(price)) {
        out.set(code.toUpperCase(), price);
      }
    }
